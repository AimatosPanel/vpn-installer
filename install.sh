...
log_step "[2/5] Обновление репозиториев и установка зависимостей..."
...
log_step "[4/5] Загрузка репозиториев с GitHub..."
rm -rf "$SRC_DIR"
mkdir -p "$SRC_DIR"
cd "$SRC_DIR"
...
log_success "Исходные коды загружены"
...
if [[ -f /tmp/aimatos-installer ]]; then
    ...
    log_success "Установщик готов к запуску"
    sleep 1
    clear
    cd /root
    ...
    echo -e "      Лог ошибки: ${CLR_YELLOW}cat $LOG_FILE${CLR_RESET}"
    exit 1
fi